...
func CreateSession(c common2.Client, uri, username, password string) (auth *AuthToken, err error) {
	a := &authPayload{
		UserName: username,
		Password: password,
	}
...
func DeleteSession(c common2.Client, sessionURL string) (err error) {
...
func GetSession(c common2.Client, uri string) (*Session, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedSessions(c common2.Client, link string) ([]*Session, error) {
	var result []*Session
