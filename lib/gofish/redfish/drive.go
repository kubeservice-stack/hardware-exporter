...
type Drive struct {
	...
	// drive.
	PhysicalLocation common2.Location
...
func GetDrive(c common2.Client, uri string) (*Drive, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedDrives(c common2.Client, link string) ([]*Drive, error) { //nolint:dupl
	var result []*Drive
	if link == "" {
		return result, nil
	}
