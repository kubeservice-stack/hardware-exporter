...
func (chassis *Chassis) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		Drives          common2.Link
		Thermal         common2.Link
		Power           common2.Link
		NetworkAdapters common2.Link
		Links           linkReference
		Actions         Actions
	}
...
func GetChassis(c common2.Client, uri string) (*Chassis, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedChassis(c common2.Client, link string) ([]*Chassis, error) {
	var result []*Chassis
...
func (chassis *Chassis) Drives() ([]*Drive, error) {
	...
	if chassis.drives != "" {
		drives, err := common2.GetCollection(chassis.Client, chassis.drives)
		if err != nil {
			return nil, err
		}
...
func (chassis *Chassis) NetworkAdapters() ([]*NetworkAdapter, error) {
	//fmt.Println("chassis.networkAdapters===========1", chassis.networkAdapters)
