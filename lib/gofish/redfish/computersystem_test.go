...
func TestComputerSystem(t *testing.T) { // nolint
	...
	if result.ProcessorSummary.Status.State != common2.EnabledState {
	...
	if result.MemorySummary.Status.State != common2.EnabledState {
	...
	if result.TrustedModules[0].Status.Health != common2.OKHealth {
...
func TestComputerSystemUpdate(t *testing.T) {
	...
	testClient := &common2.TestClient{}
	result.SetClient(testClient)

	result.AssetTag = TestAssetTag
	result.HostName = "TestHostName"
	result.IndicatorLED = common2.BlinkingIndicatorLED
	err = result.Update()
