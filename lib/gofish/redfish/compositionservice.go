...
func (compositionservice *CompositionService) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		ResourceBlocks common2.Link
		ResourceZones  common2.Link
	}
...
func GetCompositionService(c common2.Client, uri string) (*CompositionService, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedCompositionServices(c common2.Client, link string) ([]*CompositionService, error) { //nolint:dupl
