...
type Bios struct {
	...
	settingsApplyTimes []common2.ApplyTime
...
func (bios *Bios) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		...
		Actions  Actions
		Links    Links
		Settings common2.Settings `json:"@Redfish.Settings"`
	}
...
func GetBios(c common2.Client, uri string) (*Bios, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedBioss(c common2.Client, link string) ([]*Bios, error) { //nolint:dupl
	var result []*Bios
	if link == "" {
		return result, nil
	}
...
func (bios *Bios) AllowedAttributeUpdateApplyTimes() []common2.ApplyTime {
	if len(bios.settingsApplyTimes) == 0 {
		result := []common2.ApplyTime{
			common2.ImmediateApplyTime,
			common2.OnResetApplyTime,
			common2.AtMaintenanceWindowStartApplyTime,
			common2.InMaintenanceWindowOnResetApplyTime,
		}
		return result
	}
