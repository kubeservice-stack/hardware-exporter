...
func (logentry *LogEntry) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		...
		Links struct {
			// OriginOfCondition shall be an href that
			// references the resource for which the log is associated.
			OriginOfCondition common2.Link
		}
	}
...
func GetLogEntry(c common2.Client, uri string) (*LogEntry, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedLogEntrys(c common2.Client, link string) ([]*LogEntry, error) { //nolint:dupl
	var result []*LogEntry
	if link == "" {
		return result, nil
	}
