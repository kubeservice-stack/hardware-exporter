...
type Endpoint struct {
	...
	EndpointProtocol common2.Protocol
...
func GetEndpoint(c common2.Client, uri string) (*Endpoint, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedEndpoints(c common2.Client, link string) ([]*Endpoint, error) { //nolint:dupl
	var result []*Endpoint
	if link == "" {
		return result, nil
	}
...
type IPTransportDetails struct {
	...
	TransportProtocol common2.Protocol
}
