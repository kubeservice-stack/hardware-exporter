...
func GetThermal(c common2.Client, uri string) (*Thermal, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedThermals(c common2.Client, link string) ([]*Thermal, error) { //nolint:dupl
	var result []*Thermal
	if link == "" {
		return result, nil
	}
