...
func (manager *Manager) UnmarshalJSON(b []byte) error {
	...
	type linkReference struct {
		ManagerForChassis       common2.Links
		ManagerForChassisCount  int `json:"ManagerForChassis@odata.count"`
		ManagerForServers       common2.Links
		ManagerForServersCount  int `json:"ManagerForServers@odata.count"`
		ManagerForSwitches      common2.Links
		ManagerForSwitchesCount int `json:"ManagerForSwitches@odata.count"`
		ManagerInChassis        common2.Link
		Oem                     json.RawMessage
	}
	var t struct {
		temp
		EthernetInterfaces   common2.Link
		HostInterfaces       common2.Link
		LogServices          common2.Link
		NetworkProtocol      common2.Link
		RemoteAccountService common2.Link
		SerialInterfaces     common2.Link
		VirtualMedia         common2.Link
		Links                linkReference
		Actions              actions
		Oem                  json.RawMessage
	}
...
func GetManager(c common2.Client, uri string) (*Manager, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedManagers(c common2.Client, link string) ([]*Manager, error) {
	var result []*Manager
