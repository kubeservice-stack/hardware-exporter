...
func GetTask(c common2.Client, uri string) (*Task, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedTasks(c common2.Client, link string) ([]*Task, error) { //nolint:dupl
	var result []*Task
	if link == "" {
		return result, nil
	}
