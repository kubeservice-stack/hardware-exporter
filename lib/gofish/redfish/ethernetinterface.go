...
const (
	// ZteLinkUpLinkStatus The link is available for communication on this
	// interface.
	ZteLinkUpLinkStatus LinkStatus = "Up"
	// ZteNoLinkLinkStatus There is no link or connection detected on this
	// interface.
	ZteNoLinkLinkStatus LinkStatus = "No"
	// ZteLinkDownLinkStatus There is no link on this interface, but the
	// interface is connected.
	ZteLinkDownLinkStatus LinkStatus = "Down"
)
...
func (ethernetinterface *EthernetInterface) UnmarshalJSON(b []byte) error {
	...
	type links struct {
		Chassis        common2.Link
		Endpoints      common2.Links
		EndpointsCount int `json:"Endpoints@odata.count"`
		HostInterface  common2.Link
	}
...
func GetEthernetInterface(c common2.Client, uri string) (*EthernetInterface, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedEthernetInterfaces(c common2.Client, link string) ([]*EthernetInterface, error) { //nolint:dupl
