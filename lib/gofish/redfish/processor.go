...
func (processor *Processor) UnmarshalJSON(b []byte) error {
	...
	type t1 struct {
		temp
		AccelerationFunctions common2.Link
		Assembly              common2.Link
		Metrics               common2.Link
		ProcessorMemory       common2.Links
		Links                 struct {
			Chassis                  common2.Link
			ConnectedProcessors      common2.Links
...
func GetProcessor(c common2.Client, uri string) (*Processor, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedProcessors(c common2.Client, link string) ([]*Processor, error) {
