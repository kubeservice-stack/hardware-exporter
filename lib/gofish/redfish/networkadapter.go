...
func (networkadapter *NetworkAdapter) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		...
		NetworkDeviceFunctions common2.Link
		NetworkPorts           common2.Link
		Actions                actions
	}
...
func GetNetworkAdapter(c common2.Client, uri string) (*NetworkAdapter, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedNetworkAdapter(c common2.Client, link string) ([]*NetworkAdapter, error) {
...
func (networkadapter *NetworkAdapter) NetworkPorts() ([]*NetworkPort, error) {
	//fmt.Println("networkadapter.networkPorts**********1", networkadapter.networkPorts)
