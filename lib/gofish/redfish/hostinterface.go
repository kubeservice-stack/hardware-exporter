...
func (hostinterface *HostInterface) UnmarshalJSON(b []byte) error {
	...
	type links struct {
		AuthNoneRole         common2.Link
		ComputerSystems      common2.Links
		ComputerSystemsCount int `json:"ComputerSystems@odata.count"`
		FirmwareAuthRole     common2.Link
		KernelAuthRole       common2.Link
	}

	var t struct {
		temp
		Links                    links
		HostEthernetInterfaces   common2.Link
		ManagerEthernetInterface common2.Link
...
func GetHostInterface(c common2.Client, uri string) (*HostInterface, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedHostInterfaces(c common2.Client, link string) ([]*HostInterface, error) { //nolint:dupl
