...
func (networkdevicefunction *NetworkDeviceFunction) UnmarshalJSON(b []byte) error {
	...
	type links struct {
		...
		PCIeFunction           common2.Link
		PhysicalPortAssignment common2.Link
	}
	var t struct {
		temp
		Links                   links
		AssignablePhysicalPorts common2.Links
	}
...
func GetNetworkDeviceFunction(c common2.Client, uri string) (*NetworkDeviceFunction, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedNetworkDeviceFunctions(c common2.Client, link string) ([]*NetworkDeviceFunction, error) { //nolint:dupl
