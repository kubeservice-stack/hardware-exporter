...
func GetPower(c common2.Client, uri string) (*Power, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedPowers(c common2.Client, link string) ([]*Power, error) { //nolint:dupl
	var result []*Power
	if link == "" {
		return result, nil
	}
...
type PowerControl struct {
	...
	PhysicalContext common2.PhysicalContext
