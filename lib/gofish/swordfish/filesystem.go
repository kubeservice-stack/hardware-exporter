...
func (filesystem *FileSystem) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		ExportedShares common2.Link
		ReplicaTargets common2.Links
		Links          links
	}
...
func GetFileSystem(c common2.Client, uri string) (*FileSystem, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedFileSystems(c common2.Client, link string) ([]*FileSystem, error) { //nolint:dupl
