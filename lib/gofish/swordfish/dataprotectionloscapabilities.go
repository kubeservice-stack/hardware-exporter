...
func (dataprotectionloscapabilities *DataProtectionLoSCapabilities) UnmarshalJSON(b []byte) error {
	...
	type DLinks struct {
		...
		SupportedReplicaOptions common2.Links
		// SupportedReplicaOptionsCount is the number of supported replica options.
	...
	var t struct {
		temp
		Links                   DLinks
		SupportedLinesOfService common2.Links
	}
...
func GetDataProtectionLoSCapabilities(c common2.Client, uri string) (*DataProtectionLoSCapabilities, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedDataProtectionLoSCapabilities(c common2.Client, link string) ([]*DataProtectionLoSCapabilities, error) { //nolint:dupl
