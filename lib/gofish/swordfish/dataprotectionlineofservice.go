...
type DataProtectionLineOfService struct {
	...
	ReplicaAccessLocation common2.Location
	...
	// the schedule.
	Schedule common2.Schedule
}

// GetDataProtectionLineOfService will get a DataProtectionLineOfService instance from the service.
func GetDataProtectionLineOfService(c common2.Client, uri string) (*DataProtectionLineOfService, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedDataProtectionLineOfServices(c common2.Client, link string) ([]*DataProtectionLineOfService, error) { //nolint:dupl
