...
type IOConnectivityLoSCapabilities struct {
	...
	SupportedAccessProtocols []common2.Protocol
...
func GetIOConnectivityLoSCapabilities(c common2.Client, uri string) (*IOConnectivityLoSCapabilities, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedIOConnectivityLoSCapabilitiess(c common2.Client, link string) ([]*IOConnectivityLoSCapabilities, error) { //nolint:dupl
