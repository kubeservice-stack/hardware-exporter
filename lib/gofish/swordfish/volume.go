...
type Volume struct {
	...
	EncryptionTypes []redfish2.EncryptionTypes
...
func (volume *Volume) UnmarshalJSON(b []byte) error {
	...
	type links struct {
		...
		DedicatedSpareDrives common2.Links
	...
	var t struct {
		temp
		AllocatedPools common2.Links
		StorageGroups  common2.Links
		Links          links
		Actions        actions
	}
...
func GetVolume(c common2.Client, uri string) (*Volume, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedVolumes(c common2.Client, link string) ([]*Volume, error) { //nolint:dupl
	var result []*Volume
	if link == "" {
		return result, nil
	}
...
func (volume *Volume) getDrives(links []string) ([]*redfish2.Drive, error) {
	var result []*redfish2.Drive

	collectionError := common2.NewCollectionError()
	for _, driveLink := range links {
		drive, err := redfish2.GetDrive(volume.Client, driveLink)
		if err != nil {
