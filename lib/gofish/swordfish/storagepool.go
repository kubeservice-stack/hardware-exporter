...
import (
	...
	"hardware_exporter/lib/gofish/redfish"
	"reflect"
)
...
func (storagepool *StoragePool) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		Links                 links
		AllocatedPools        common2.Link
		AllocatedVolumes      common2.Link
		CapacitySource        common2.Links
...
func GetStoragePool(c common2.Client, uri string) (*StoragePool, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedStoragePools(c common2.Client, link string) ([]*StoragePool, error) { //nolint:dupl
