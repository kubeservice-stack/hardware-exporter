...
func (storageservice *StorageService) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		ClassesOfService              common2.Link
		DataProtectionLoSCapabilities common2.Link
		DataSecurityLoSCapabilities   common2.Link
		DataStorageLoSCapabilities    common2.Link
		DefaultClassOfService         common2.Link
		Drives                        common2.Link
		EndpointGroups                common2.Link
		Endpoints                     common2.Link
		FileSystems                   common2.Link
		IOConnectivityLoSCapabilities common2.Link
		IOPerformanceLoSCapabilities  common2.Link
		Redundancy                    common2.Links
		SpareResourceSets             common2.Links
		StorageGroups                 common2.Link
		StoragePools                  common2.Link
		StorageSubsystems             common2.Link
		Volumes                       common2.Link
		Links                         links
		Actions                       actions
	}
...
func GetStorageService(c common2.Client, uri string) (*StorageService, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedStorageServices(c common2.Client, link string) ([]*StorageService, error) {
...
func (storageservice *StorageService) Drives() ([]*redfish2.Drive, error) {
	return redfish2.ListReferencedDrives(storageservice.Client, storageservice.drives)
}
...
func (storageservice *StorageService) Endpoints() ([]*redfish2.Endpoint, error) {
	return redfish2.ListReferencedEndpoints(storageservice.Client, storageservice.endpoints)
}
...
func (storageservice *StorageService) Redundancy() ([]*redfish2.Redundancy, error) {
	var result []*redfish2.Redundancy

	collectionError := common2.NewCollectionError()
	for _, redundancyLink := range storageservice.redundancy {
		redundancy, err := redfish2.GetRedundancy(storageservice.Client, redundancyLink)
		if err != nil {
