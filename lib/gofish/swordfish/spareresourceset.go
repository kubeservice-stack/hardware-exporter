...
type SpareResourceSet struct {
	...
	OnHandLocation common2.Location
...
func (spareresourceset *SpareResourceSet) UnmarshalJSON(b []byte) error {
	...
	type links struct {
		OnHandSpares common2.Links
		// OnHandSparesCount is the number of on hand spares.
		OnHandSparesCount int `json:"OnHandSpares@odata.count"`
		// ReplacementSpareSets are other spare sets that can be utilized to
		// replenish this spare set.
		ReplacementSpareSets common2.Link
...
func GetSpareResourceSet(c common2.Client, uri string) (*SpareResourceSet, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedSpareResourceSets(c common2.Client, link string) ([]*SpareResourceSet, error) { //nolint:dupl
