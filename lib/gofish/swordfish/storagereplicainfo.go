...
func (replicainfo *ReplicaInfo) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		DataProtectionLineOfService common2.Links
		Replica                     common2.Link
	}
...
func GetStorageReplicaInfo(c common2.Client, uri string) (*StorageReplicaInfo, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedStorageReplicaInfos(c common2.Client, link string) ([]*StorageReplicaInfo, error) { //nolint:dupl
