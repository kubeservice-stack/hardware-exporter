...
func (storagegroup *StorageGroup) UnmarshalJSON(b []byte) error {
	...
	var t struct {
		temp
		Links                links
		ServerEndpointGroups common2.Links
		Actions              actions
	}
...
func GetStorageGroup(c common2.Client, uri string) (*StorageGroup, error) {
	resp, err := c.Get(uri)
	if err != nil {
		return nil, err
	}
...
func ListReferencedStorageGroups(c common2.Client, link string) ([]*StorageGroup, error) { //nolint:dupl
